@php
    ...
    $profileLinks = [
        ['name' => 'Profile',      'route' => 'panel.profile'],
        ['name' => 'Settings',     'route' => 'panel.settings'],
        ...
        <div class="flex h-16 items-center justify-between gap-x-4">
            ...
            <div class="relative w-72" x-data="{ open: false }">
                ...
                <div 
                    ...
                    x-cloak
                >
                    @if(empty($searchResults['products']) && empty($searchResults['orders']) && empty($searchResults['users']))
                        <p class="text-slate-400 text-center py-2">No results found for "{{ $searchQuery }}"</p>
                    @else
                        {{-- Products --}}
                        @if(!empty($searchResults['products']))
                            <div class="mb-3">
                                ...
                                @foreach($searchResults['products'] as $product)
                                    <a href="{{ route('admin.products.index') }}" class="block py-1.5 hover:text-indigo-400 truncate">
                                        {{ $product->name }}
                                    </a>
                                @endforeach
                            </div>
                        ...
                        @if(!empty($searchResults['orders']))
                            <div class="mb-3">
                                ...
                                @foreach($searchResults['orders'] as $order)
                                    <a href="{{ route('admin.orders.index') }}" class="flex justify-between py-1.5 hover:text-indigo-400">
                        ...
                        @if(!empty($searchResults['users']))
                            <div>
                                <span class="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Users</span>
                                @foreach($searchResults['users'] as $user)
                                    <a href="{{ route('admin.users.index') }}" class="flex justify-between py-1.5 hover:text-indigo-400">
                                        <span>{{ $user->name }}</span>
                                        <span class="text-slate-400 text-[10px]">{{ $user->role }}</span>
                                    </a>
                                @endforeach
                            </div>
                        @endif
