    ...
    >
        <x-slot:data>
            ...
            @forelse($categories as $cat)
                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-slate-900/30 transition">

                            <td class="px-6 py-4">
                                <div class="font-semibold text-gray-600 dark:text-slate-200">{{ $cat->name }}</div>
                                <div class="text-xs text-gray-500 dark:text-slate-500 mt-0.5">
                                    Status Flag: {{ $cat->is_visible ? '🟢 Public View' : '🔴 Internal Sandbox Only' }}
                                </div>
                            </td>

                            <td class="px-6 py-4 font-mono text-xs text-gray-500 dark:text-slate-400">{{ $cat->slug }}</td>
                            
                            <td class="px-6 py-4">
                                @if($cat->parent)
                                    <span class="px-2 py-1 text-xs font-semibold bg-slate-800 text-indigo-300 rounded">Sub of {{ $cat->parent->name }}</span>
                                @else
                                    <span class="px-2 py-1 text-xs font-semibold bg-indigo-950 text-indigo-300 rounded">Root Level Node</span>
                                @endif
                            </td>
                            
                            <td class="px-6 py-4 text-center text-gray-500 dark:text-slate-400">{{ $cat->products_count }} items</td>
                            
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2 flex">

                                <x-button.edit action="{{ route('panel.categories.edit', $cat) }}"></x-button.edit>
                                
                                <x-button.delete action="deleteCategory({{ $cat->id }})"></x-button.delete>
